...
public class TableProcessServiceImpl implements TableProcessService {
    ...
    @Autowired
    public ParamInfoDao paramInfoDao;

    @Override
    public boolean tableInfoFilterProcess() {
        String tableName = CoreApplication.tableNameList.get(0);
        ...
        List<Map<String,Object>> list = jdbcTemplate.queryForList("select * from "+tableName+" where is_filtered=0 limit 20");
        if(list==null){
